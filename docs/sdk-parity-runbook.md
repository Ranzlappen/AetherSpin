...
| **FAIL: bonus RTP** or **FAIL: multiplierWilds** | The free-game reconciliation regressed (ADR 0005).                                  | Re-check `sample_multiplier_grid` + the summed per-cell wild logic in `math/games/<id>/game_calculations.py`; confirm `reveal_event` emits `multiplierWilds`. `test_sdk_book_contract.py` / `test_parity.py` should also be red. |
