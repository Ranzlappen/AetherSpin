...
| **Win-tier overlay plates**   | `public/fx/win-<tier>.webp` (big/mega/epic/wincap)           | WebP/PNG, transparent        | ~640×360 → **~1280×720**                | The tiered BIG/MEGA/EPIC/MAX WIN overlay (`components/WinCelebration.svelte`) is CSS-only today; it exposes a `data-tier` styling hook so plates/headline art swap in per tier. |
