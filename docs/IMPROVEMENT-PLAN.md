...
| B6  | **CI hardening.** Switch to `--frozen-lockfile`; **stop masking ESLint failures**; add `gitleaks` + CodeQL (TS+Python) + dependency-review; **matrix CI over every `shared/games/*`** (not just NovaForge).          | `.github/workflows/*`                                                   | S/M    | 5/7       |
...
| C5  | **Engine generalization + 2nd game type.** Extract `BaseEngine`; add a minimal **ways** (or cluster) example; make `new-game.sh` scaffold from declared mechanics, not a NovaForge clone.                                            | `math/simulator/engine.py`, `scripts/new-game.sh`, `shared/games/` | L      | 6/7       |
| C6  | **Multi-game frontend.** `VITE_GAME_ID` build-time selection + per-game theme/asset namespaces so new titles need no core source edits (`gameConfig.ts` currently hardcodes NovaForge).                                              | `frontend/src/config/gameConfig.ts`, `vite.config.ts`              | M/L    | 2/7       |
...
runs**, for **all three** games, not just NovaForge.

- **Real-SDK ports for all three games** — NovaForge (`lines`), Cosmic Ways
