...
class LinesEngine:
    ...
    # ---- evaluation ---------------------------------------------------------
    def _evaluate_lines(self, board: list[list[str]], free: bool) -> tuple[float, list[dict[str, Any]]]:
        total = 0.0
        wins: list[dict[str, Any]] = []
        for line_idx, pattern in enumerate(self.paylines):
            symbols = [board[reel][pattern[reel]] for reel in range(self.d.num_reels)]
            win = self._evaluate_single_line(symbols, free)
            if win is None:
                continue
    ...
    def _evaluate_single_line(self, symbols: list[str], free: bool):
        """Left-aligned line win. Returns (symbol, count, base_value, wild_mult)."""
        first = symbols[0]
        # Determine the paying symbol: first non-wild, or wild if the line opens
        # with wilds that are never resolved by a paid symbol.
        pay_symbol = None
        for s in symbols:
            if s == self.scatter:
                break
        ...
        if pay_symbol is None:
            # all leading symbols are wild -> pay as wild line
        ...
        for s in symbols:
            if s == pay_symbol or s == self.wild:
                ...
                break
        # Multiplier wilds apply only in the free game and do NOT stack
        # multiplicatively across a line (we apply a single line multiplier) to
        # keep volatility bounded and RTP tractable.
        wild_mult = self._expected_wild_mult if (free and has_wild) else 1
    ...
    def play_round(self, sim_id: int, rng: Rng, force_free: bool = False, disable_free: bool = False) -> SpinResult:
        # Precompute expected wild multiplier (weighted) for line evaluation.
        vals, wts = self.d.mult_wild_values, self.d.mult_wild_weights
        self._expected_wild_mult = max(1, round(sum(v * w for v, w in zip(vals, wts)) / sum(wts)))
        ...
        line_total, line_wins = self._evaluate_lines(board, free=False)
        if line_wins:
    ...
    def _run_freegame(self, scatter_count: int, rng: Rng, events: list[dict[str, Any]]) -> float:
        ...
        while spins_done < spins_total:
            ...
            line_total, line_wins = self._evaluate_lines(board, free=True)
