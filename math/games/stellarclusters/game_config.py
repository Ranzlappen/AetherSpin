"""Stellar Clusters config for the official StakeEngine math-sdk (real API).

Ported to the current SDK `Config` API (see ADR 0005), modelled on NovaForged
and the SDK's `0_0_cluster` example (single evaluation, no tumble — matching the
standalone `ClusterMechanic`). Numbers load from the canonical
`shared/games/stellarclusters/game-definition.json`.

Mechanic: 5x3 cluster-pays (orthogonally-connected groups of >= 3, wilds
substitute). Multiplier wilds and expanding wilds are disabled for this game, so
the free game is cluster + scatter pays, scaled by `features.freeSpins.winScale`.
"""
...
class GameConfig(Config):
    """Stellar Clusters — 5x3 cluster-pays neon slot (official math-sdk path)."""
    ...
    def __init__(self) -> None:
        ...
        self.working_name = "StellarClusters"
        ...
        self.paylines = {}  # cluster games have no fixed paylines
        ...
        self.contract_win_event = "clusterWins"
