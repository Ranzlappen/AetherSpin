...
The game ships with **placeholder art and audio** — SVG symbols in
`public/symbols/` (generated by `scripts/gen-placeholder-art.mjs`) and
synthesized WAV SFX in `public/audio/` (`scripts/gen-placeholder-audio.mjs`),
declared in `ASSET_MANIFEST` (`src/config/assets.ts`). Every asset is optional:
a missing or undecodable file is skipped and the renderer falls back to
procedural Pixi `Graphics`/`Text`, so replacing placeholders with final art is
a pure file drop-in (see `docs/asset-spec.md` for exact deliverables).
