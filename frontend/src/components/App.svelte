<script lang="ts">
  ...
  let loading = true;
  let loadingMessage = 'Initializing NovaForged…';
  let usingMock = false;
  ...
  async function authenticate(): Promise<void> {
    if (!transport) return;
    loading = true;
    loadingMessage = usingMock ? 'Starting local demo session…' : 'Authenticating…';
    try {
  ...
  function handleSpinError(err: unknown): void {
    stopAutoplay();
    const message = err instanceof RgsError ? err.message : ((err as Error)?.message ?? 'Spin failed.');
    errorMessage.set(message);
  }
  ...
  async function runAutoplay(token: number): Promise<void> {
    while (token === autoplayToken && get(autoplay).active && get(autoplay).remaining > 0) {
      if (get(insufficientFunds)) {
        errorMessage.set('Autoplay stopped: insufficient balance.');
        break;
      }
      await spin('base');
...
<div class="game-root">
  ...
  {#if !loading}
    ...
    <header class="hud-top">
      ...
      <div class="top-right">
        <SoundToggle />
        <button class="icon-btn" aria-label="Paytable" on:click={() => (paytableOpen = true)}> i </button>
      </div>
    </header>
    ...
    {#if usingMock}
      <div class="dev-badge" title="No RGS params detected — running on the local mock RGS">
        DEMO · mock RGS · {fps.toFixed(0)} fps
      </div>
    {/if}
  {/if}
  ...
  {#if $errorMessage}
    <div class="toast panel" role="alert">
      ...
      <button class="toast-close" on:click={dismissError} aria-label="Dismiss">✕</button>
    </div>
  {/if}
