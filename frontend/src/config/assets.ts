...
/**
 * Shared **final** neon-cosmic symbol art (512×512 WebP with alpha), keyed by
 * symbol id. All three games share this set today because they share the same
 * symbol ids. It's the fallback every game resolves to unless it overrides a
 * symbol in {@link THEMED_SYMBOL_SETS}.
 */
...
/**
 * Scene art rendered by Pixi: the nebula background plate and the reel-board
 * frame. Like symbols, each is optional — a missing texture falls back to the
 * procedural rendering (gradient nebula / neon frame).
 */
 ...
 * one exists, otherwise the shared set. Defaults to the active game.
 */
 ...
 */
export const ASSET_MANIFEST: readonly AssetGroup[] = [symbolsGroup(), { name: 'scene', assets: SCENE_ART }];
